...
- **Master dev accounts use `.local` emails** (`dev-shelter@fostrfix.local` / `dev-foster@fostrfix.local`). These are invalid at the DNS level. Resend will REJECT these if you try to send real emails with a real API key — you'll get `Email address "dev-shelter@fostrfix.local" is invalid` in the server logs, and the UI will show the account-enumeration-safe generic success state. **If you need real end-to-end email testing, either:** (a) ask the user for Option B (add `DEV_OVERRIDE_EMAIL` rewriter), (b) re-run `scripts/setup-master-accounts.mjs` with real email addresses, or (c) verify a real sending domain in Resend and change the master emails.
