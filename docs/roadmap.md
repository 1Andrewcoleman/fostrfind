...
- All API routes under `src/app/api/` — add rate limiting. The current roster (keep in sync as new routes land):
  ...
  - `POST   /api/reports`                    (Phase 6.4 — `reports:post`, 5/min per user — reporting is rare; low cap blocks scripted spam without affecting real users)
  - `POST   /api/dogs/[id]/save`             (Phase 6.5 — `dogs:save`, 60/min per foster — heart toggle UX needs headroom for fast clicks)
  - `DELETE /api/dogs/[id]/save`             (Phase 6.5 — `dogs:unsave`, 60/min per foster)
...
**Status:** ✅ Shipped 2026-04-20 — `3b1750e` (shelter deep-link filter on browse), `5a9c13b` (public `/shelters` index with name search). `FilterState` gained an optional `shelter: string | null` field wired into the browse page's filter predicate + active-filter chip; the helper lives in `src/lib/browse-filters.ts` with vitest coverage. `/shelters` is a server page that lists partnered shelters alphabetically, supports a JS-free GET search form (`?q=` with Postgres `ilike` + wildcard escaping), reuses the existing `/shelters/[slug]` `PLACEHOLDER_SHELTERS` fixtures in DEV_MODE, and links each card to both the profile and `/foster/browse?shelter=<slug>`. Deferred follow-ups (server-side shelter filter on browse, shelter-name autocomplete, `/shelters` pagination beyond 200) are logged below.

### 6.2 Shelter-side foster roster (history + invites + notes)

**Intent:** Shelters maintain an explicit **roster** of fosters they work with. Membership is driven by two paths only — automatic add when an application transitions to `accepted`, and a shelter-initiated **email invite** that works even if the foster hasn't signed up yet. Foster-side transparency: see which shelters have them on a roster and remove themselves at will.

**Relationship to existing work:** New surface under `/shelter/fosters` (index + per-foster detail) and two new foster-side pages (`/foster/invites`, `/foster/shelters-roster`). No global foster directory; no messaging outside applications in v1. The roster schema is intentionally forward-compatible with a future **Herds** emergency-group feature.

**Status:** ✅ Shipped 2026-04-22 — 9 commits across a fully additive migration + new API surfaces + new portal pages:

- `1b316fe` migration (`shelter_fosters`, `shelter_foster_invites`, `shelter_foster_notes`) with RLS, indexes, and a `scripts/verify-phase-6-62-migrations.sql` probe.
- `a50bbf5` row types in `src/types/database.ts`, `src/lib/shelter-roster.ts` helpers (`normalizeInviteEmail`, `activeFosteringCount`) + 9 unit tests, `src/lib/supabase/service.ts` `createServiceClient()`, and an import-allowlist test that enforces the service client is only imported from the two pre-approved API routes.
- `d4d46f4` auto-add upsert in `POST /api/applications/[id]/accept` (wrapped — soft-failure + idempotent); onboarding email-match claim in the foster-parents insert path, under the caller's JWT.
- `7d191ee` four invite API routes (`POST /api/shelter/foster-invites` + `[id]/accept` + `[id]/decline` + `[id]/cancel`), plus `ShelterFosterInviteEmail` wired through the existing notifications dispatcher; 25 new tests covering pre-signup, double-accept, RLS cross-shelter rejection, and soft-failure of the service-role upsert.
- `d0a329b` `DELETE /api/foster/shelter-roster/[shelterId]` self-remove endpoint + 6 tests.
- `27b9bed` `/foster/invites` page + accept/decline client actions; `FosterPortalShell` grew an optional `pendingInvites` prop; `portal-nav.tsx` switched from a boolean `showUnreadBadge` to a `badgeKey` discriminator so multiple counters can drive sidebar badges; `getPortalLayoutData` now returns a `pendingInvites` count. Also fixed a pre-existing Next.js route-file export violation by moving `PLACEHOLDER_SHELTERS` to `src/lib/placeholder-shelters.ts`.
- `9a453b7` `/foster/shelters-roster` transparency page with a per-row Leave-roster AlertDialog; dashboard discovery card.
- `ee9d077` `/shelter/fosters` index with roster-scoped name search, single grouped `activeFosteringCount` query, invite form card, pending invites card with inline Cancel, and sidebar nav entry.
- `<this commit>` `/shelter/fosters/[fosterId]` detail page (public profile block, home/preferences, per-shelter interaction history, private notes) + `POST /api/shelter/foster-notes` with 6 tests + `shelter_foster_notes` RLS verified.

Design decisions worth preserving for future contributors:

- `shelter_fosters` uses a composite PK `(shelter_id, foster_id)` — not `id uuid` — so the future `herd_members` table can FK straight to it without reshape.
- `shelter_fosters` has **no INSERT policy**. Membership writes go through the service-role client in exactly two API routes (the accept upsert and the invite-accept insert). Enforcing "only two entry paths" in a single place, rather than duplicating the gate in RLS + app code, is the point of that asymmetry.
- The partial unique index `(shelter_id, lower(email)) WHERE status = 'pending'` allows one pending invite per pair while leaving declined/cancelled history unconstrained, so a shelter can legitimately re-invite after a decline or cancellation.
- "Active fostering count" is a platform-wide count, intentionally — it exposes a number, never which shelters. Computed via one grouped query on the index page and a `count: 'exact'` head query on the detail page.
- Roster self-removal is decoupled from application status. Leaving a roster doesn't cancel any in-flight applications; the Remove button's dialog copy says so.

Deferred follow-ups (below) cover the larger Herds buildout, shelter-initiated removal, generic notifications, SMS invites, note edit/delete, and note audit log.
...
**Status:** ✅ Shipped 2026-04-20 — `0bcd563` (extract `FosterPortalShell`), `c9bc7ce` (move `/foster/dog/[id]` out of the `(foster)` route group), `4c18916` (`ShareButton` with clipboard fallback), `0d4825a` (public teaser replacing the logged-out redirect + `robots.txt` allow carve-out + teaser-safe `generateMetadata`). The route now serves a public teaser (no apply dialog, no medical/temperament/special-needs blocks, no "already applied" indicator) on the same canonical URL for anonymous visitors and non-foster logged-in users; authenticated fosters still get the full detail with apply dialog and shelter rating, rendered inside `FosterPortalShell` so sidebar/mobile nav/DEV_MODE banner/theme toggle all match the rest of the portal. `ShareButton` uses `navigator.share` with an `AbortError` swallow and a clipboard + `sonner` toast fallback. Deferred follow-ups (share analytics, `og:image`, sitemap entries) are logged below.
...
**Status:** ✅ Shipped 2026-04-29 — migration `20240114000000_reports.sql` adds the `reports` table with a subject XOR check (exactly one of `subject_foster_id` / `subject_shelter_id` is non-null), `pending|reviewing|resolved|dismissed` status, plus RLS that grants SELECT only to the reporter and INSERT only when the caller is a participant (foster or shelter owner) on the referenced application AND the inferred subject column matches the OTHER party. The route `POST /api/reports` (`src/app/api/reports/route.ts`) follows the same Zod + getUser + rate-limit + sanitize pattern as `/api/ratings`; subject ids are derived server-side, never sent by the client. Idempotency is on PENDING reports per (application, reporter) so a triaged report can be re-filed if a new issue arises. UI: shared `ReportApplicationDialog` (`src/components/report-application-dialog.tsx`) with category Select + Textarea, wired into the foster `ApplicationStatusCard` action row and the shelter `/shelter/applications/[id]` footer. 13 vitest specs cover auth, rate-limit, validation, ownership, idempotency, both happy paths. Triage UI itself is deferred (see Deferred Follow-ups Log).
...
**Status:** ✅ Shipped 2026-04-29 — migration `20240115000000_dog_saves.sql` adds `dog_saves` keyed `(foster_id, dog_id)` with cascade FKs, RLS scoped via the existing `get_my_foster_ids()` helper, and a `SECURITY DEFINER` RPC `get_save_counts_for_my_dogs()` that returns `(dog_id, save_count)` for the caller's own shelters — shelters never read individual rows, only aggregates. Dual-method route `/api/dogs/[id]/save` (`POST` save, `DELETE` unsave) is idempotent on both sides and rate-limited via Step 30. Foster UI: optimistic `SaveDogButton` heart on `DogDetailFull`, new `/foster/saved` page (server-rendered via the dog_saves → dogs join, reuses `BrowseDogCard`), and a `Heart` nav item between Browse and My Applications. Shelter UI: `ShelterDogsTabs` calls the RPC once and threads the count map into `DogCard`, which now shows `N save(s)` alongside the application count on the active tab. Per the `.impeccable.md` "no placeholder metrics" guardrail, `saveCount` is always a real DB aggregate (defaults to 0, never invented). Seed script writes one save per (foster × first-dog-of-shelter) so QA sees non-zero counts on day one.
...
**Status:** ⏸ Deferred 2026-04-29 — product call documented while wrapping §6.4 and §6.5. The blocker is the same one Step 22 flagged: there is no automated geocoding pipeline, so `shelters.latitude` / `shelters.longitude` and `foster_parents.latitude` / `foster_parents.longitude` are populated only when manually entered (e.g. the dev seed sets coords on the three seeded shelters; production shelters can have NULL). A map UI on top of inconsistent coordinates would either silently drop shelters or fall back to a list, neither of which is a better experience than the existing list-only browse with `haversineMiles` distance filtering. Pre-conditions before this picks up: (a) decide on a geocoder (Mapbox forward-geocoding, Google Geocoding, or Supabase pg_extension), (b) wire onboarding/settings to capture and geocode `location` server-side, (c) a one-shot backfill for existing rows. After those land, the map itself is straightforward — secondary tab on `/foster/browse` with markers for shelters whose coords are non-null, fit-bounds when the foster has coords, list fallback otherwise. Logged in the Deferred Follow-ups Log so the precondition list isn't lost.
...
| 2026-04-20 | Phase 6 §6.1 (Shelter Discovery) | Shelter filter is applied **client-side** against already-loaded dogs | Phase 3 / §27 | `filters.shelter` is a single-shelter slug match in the browse grid's `filteredDogs` memo. Deep-linking `/foster/browse?shelter=<slug>` from `/shelters` or a profile works out of the box because browse loads PAGE_SIZE=24 dogs at a time ordered by shelter + recency — but a foster with many shelter partners could see 0 matches until they "Load more". Mirrors the Step 13 / Step 22 client-side-filter precedent; the real fix is a server-side `eq('shelter_id', …)` clause on the browse fetch. Coupled with the long-standing need for server-side browse filtering generally. |
| 2026-04-20 | Phase 6 §6.1 (Shelter Discovery) | No shelter-name **autocomplete** on the browse filter sidebar | Phase 3 / §27 | Only deep-link entry is wired; `FilterSidebar` intentionally has no `shelter` input in v1 (doing the typeahead right needs a dedicated `/api/shelters/search` endpoint or client-side fuzzy over a cached shelter list). When the server-side filter lands, add the typeahead at the same time so the UI matches the new query shape. |
...
| 2026-04-22 | Phase 6 §6.2 (Roster) | **Direct messaging outside applications** — "ping this foster about dog X" without requiring them to apply first | future phase | Current `messages` table is scoped by `application_id`. The v1 cut respects that boundary on purpose (messaging surface = application thread) so a shelter can't cold-contact a foster through the app. When Herds or a direct-message feature lands, decide whether to add a second message table or widen the existing one with an optional context type (application / herd / direct). |
| 2026-04-22 | Phase 6 §6.2 (Roster) | **Shelter-initiated removal/archive** of a foster from their own roster | unscheduled | v1 only exposes foster-initiated removal so the shelter-side UI stays simple and avoids a "ghost the foster" footgun. When shelter-initiated removal is wanted, add a DELETE endpoint mirroring `/api/foster/shelter-roster/[shelterId]` + a shelter-side archive tab. Requires a new INSERT/DELETE policy on `shelter_fosters` — or continuing to route through the service client (recommended) so the "two entry paths" gate stays centralised. |
| 2026-04-22 | Phase 6 §6.2 (Roster) | **Generic in-app notifications table** | future phase | v1 models invites as a first-class `/foster/invites` page with a sidebar count badge, because that's the single surface where invites are actionable. If we later add reminders, placement updates, or rating nudges, introduce a `notifications` table and migrate `/foster/invites` to a filtered view on top of it. |
| 2026-04-22 | Phase 6 §6.2 (Roster) | **SMS invites** | future phase | v1 is email-only. An SMS path will require Twilio (or equivalent), a phone-verified channel, and a new invite creation path that normalises by phone. `shelter_foster_invites.email` would need a nullable `phone` sibling column, or we split into a separate invite entity — decide at that point. |
| 2026-04-22 | Phase 6 §6.2 (Roster) | **Note edit / delete / audit log** | future phase | v1 allows creating notes only. When edit/delete lands, either add `updated_at`/`deleted_at` columns (already on the table — just wire them) + a new PUT/DELETE route, or keep notes append-only and render a "strike through" via a soft-delete flag. An audit log (`shelter_foster_note_audits`) is worth considering if notes ever feed back into shelter verification or reporting. |
| 2026-04-22 | Phase 6 §6.2 (Roster) | **Ghost-invite cleanup** | future phase | Pending invites never auto-expire; a shelter who invites a non-existent email leaves a `shelter_foster_invites` row that stays pending forever unless cancelled. Cheap to ignore at scale — a cron job trimming pending rows older than N days, or an explicit TTL column, solves it when the list gets noisy. |
| 2026-04-22 | Phase 6 §6.2 (Roster) | **Invite token link in email** | future phase | v1's email asks the recipient to sign in / sign up at `/foster/invites` rather than embedding a magic-link token, which keeps the copy simple and avoids a second secret to rotate. If click-through drops, introduce a signed token and a `/i/<token>` landing route that pre-accepts after login. |
| 2026-04-22 | Phase 6 §6.2 (Roster) | **Shelter-side foster portal search** — global foster directory | out of scope | Explicitly ruled out in the v1 product decision: shelters only ever see fosters who have interacted via the two entry paths. If a future product direction reopens this, it needs its own privacy review and would live as a separate opt-in surface for fosters. |
| 2026-04-29 | Phase 6 §6.4 (Reports) | **Admin triage queue / dashboard** | future phase / §6.7 | v1 stores reports with `status='pending'` and never UPDATEs them via the user-facing app. Triage today happens via service-role / direct DB access. When the admin app lands, it owns the status transitions (`pending → reviewing → resolved | dismissed`), reads reports cross-shelter, and links back to the originating application. |
| 2026-04-29 | Phase 6 §6.4 (Reports) | **Internal triage notification email** | unscheduled | Submitting a report does not currently page the team — it just persists the row. When `SUPPORT_EMAIL` is replaced with a real inbox (see the existing Step 36 note), wire `sendEmail` from the route on success with category + report id. The route is already idempotent on PENDING so a duplicate-email risk doesn't exist. |
| 2026-04-29 | Phase 6 §6.4 (Reports) | **Report subject inferred via `applications.foster_id` / `shelter_id`** | acceptable | The route loads the application, identifies the caller as foster or shelter on it, and writes the OTHER party's id. There is no path for a user to send an arbitrary `subject_id` from the client. RLS WITH CHECK additionally re-verifies the caller is a participant + the subject column matches. Logged so a later "report a stranger" flow doesn't quietly bypass that gate. |
| 2026-04-29 | Phase 6 §6.5 (Saves) | **Heart toggle on browse grid card** | Phase 5 polish | The heart only lives on `/foster/dog/[id]`. Adding it to `BrowseDogCard` requires either threading `initialSaved` per-card from the server fetch or a client-side `useEffect` that fans out `dog_saves` reads — both add scope to the browse path. Pure UX nice-to-have; defer until browse loading is touched again. |
| 2026-04-29 | Phase 6 §6.5 (Saves) | **"Saved" sort / filter on browse** | Phase 5 polish | Today fosters reach saved dogs via `/foster/saved` only. A "Saved-only" toggle on browse + a "Most-saved" sort would re-use the existing `dog_saves` data; defer until product asks. |
| 2026-04-29 | Phase 6 §6.5 (Saves) | **"Popular" badge driven by save count** | Phase 5 polish + product | The aggregate count is real, so a "Popular" badge backed by a real threshold (e.g. ≥10 saves) is consistent with the `.impeccable.md` no-fake-metrics rule. Wait until a clear product threshold is set — premature badging risks "popular" tags on every dog in a small pilot network. |
| 2026-04-29 | Phase 6 §6.6 (Map) | **No automated geocoding pipeline** | future phase (precondition for §6.6) | Map UI on top of inconsistent `latitude` / `longitude` would silently drop shelters or fall back to the list. Required pre-conditions: (a) pick a geocoder (Mapbox forward-geocoding, Google Geocoding, or Postgres + extension), (b) wire onboarding/settings to capture & geocode `location` server-side, (c) one-shot backfill of existing rows. After those land, the map itself is a small browse tab — fit-bounds when foster coords exist, list fallback otherwise. Logged here so the precondition list isn't lost. |
| 2026-04-29 | Phase 6 §6.6 (Map) | **Map provider + key (Mapbox vs Google)** | future phase | Defer until geocoding lands. Current preference: Mapbox GL JS + a public token via `NEXT_PUBLIC_MAPBOX_TOKEN` (soft-tier env in `src/lib/env.ts`), because it ships smaller and the styling is friendlier to the `.impeccable.md` palette. Google Maps would require a billing-enabled project and is overkill for a marker layer. |
